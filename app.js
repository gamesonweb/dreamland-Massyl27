...
function startGame() {
    ...
    BABYLON.SceneLoader.ImportMesh("", "./assets/", "Terrain.glb", scene, function(meshes) {
        ...
        terrainMaterial.diffuseTexture = new BABYLON.Texture("textures/CouleurTerrain.jpg", scene);
        terrainMaterial.backFaceCulling = false;
        plane.material = terrainMaterial;

        meshes.forEach(mesh => {
            if (mesh !== terrain && mesh.position.x === 0 && mesh.position.z === 0) {
        ...
        BABYLON.SceneLoader.ImportMesh("", "./assets/", "Avatar.glb", scene, function(meshes, _, __, animationGroups) {
                ...
                dreams.forEach(dream => {
                    if (dream.active && avatar.intersectsMesh(dream.mesh, false)) {
                        ...
                        setTimeout(() => {
                            ...
                            dream.active = true;
                            console.log(`🔁 Respawn ${dream.mesh.name} à : x=${pos.x.toFixed(2)}, y=${pos.y.toFixed(2)}, z=${pos.z.toFixed(2)}`);
                        }, 2000);
                    }
                });
            }
    ...
    function stopAllAnimations() {
        [stillAnim, runForwardAnim, runBackwardAnim, rightAnim, leftAnim].forEach(a => a?.stop());
    }
...
function loadDreams() {
    ...
    dreamTypes.forEach(type => {
        BABYLON.SceneLoader.ImportMesh("", "./assets/", type.model, scene,
        function(meshes) {
            const baseMesh = meshes.find(m => m instanceof BABYLON.Mesh && m.geometry);
            if (!baseMesh) {
                console.error(`Aucun mesh valide dans ${type.model}`); return;
            }
            ...
            for (let i = 0; i < type.count; i++) {
                ...
                console.log(`✅ Sphère ${type.name} ${i} à : x=${pos.x.toFixed(2)}, y=${pos.y.toFixed(2)}, z=${pos.z.toFixed(2)}`);
...
function endGame() {
    gameRunning = false;
    [stillAnim, runForwardAnim, runBackwardAnim, rightAnim, leftAnim].forEach(a => a?.stop());
    [moveForward, moveBackward, moveRight, moveLeft] = [false, false, false, false];
    ...
    scoreText.textContent = `Score final : ${score}`;
...
window.addEventListener("keydown", (e) => {
    if (e.code === "Space" && document.getElementById("instructionScreen").style.display === "block") {
